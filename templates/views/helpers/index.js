...
module.exports = function () {
  ...
  _helpers.total = function (a, b, options) {
      return a - b;
  };
	...
	_helpers.categoryList = function (categories, options) {
		...
		var output = '';
		var pathPrefix = options.hash.path ? options.hash.path : 'blog';
	...
	// create the category url for a checklist-category page
	_helpers.checklistCategoryUrl = function (categorySlug, options) {
		return ('/checklists/' + categorySlug);
	};

	// create the url for a checklist single page
	_helpers.checklistUrl = function (checklistSlug, options) {
		return ('/checklists/checklist/' + checklistSlug);
	};
	...
	* */
	...
	_helpers.notEmpty = function (a, options) {
		if (a !== '') {
			return options.fn(this);
		} else {
			return options.inverse(this);
	...
	_helpers.getTagName = function (tags, tag, options) {
		const el = tags.find(function (el) {
			return el.key === tag;
		});
